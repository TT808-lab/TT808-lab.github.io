...
function currentSpeechQueue() { const lang = languageOf(currentUnits.map(u => u.text).join('\n'), current?.sourceLanguage); const translated = $('showTranslation').checked && currentTranslation.size; const units = currentUnits.map(unit => ({ ...unit, translation: currentTranslation.get(unit.id) })); return speechItems(units, translated ? 'zh' : lang, Boolean(translated)); }
function loadVoices() { const lang = languageOf(currentUnits.map(u => u.text).join('\n'), current?.sourceLanguage); const voices = speech.voices(lang); $('voice').innerHTML = voices.length ? voices.map(v => `<option value="${escapeHtml(v.voiceURI)}">${escapeHtml(v.name)} (${escapeHtml(v.lang)})${v.localService ? ' · Local' : ''}</option>`).join('') : `<option>${text('未检测到匹配的本机音色','No matching local voice')}</option>`; const selected = speech.voice(lang); if (selected) $('voice').value = selected.voiceURI; }
