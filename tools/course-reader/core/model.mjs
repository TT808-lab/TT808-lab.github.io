...
function sha256Fallback(bytes) {
  ...
  const padded = new Uint8Array(paddedLength); padded.set(source); padded[source.byteLength] = 0x80;
  const view = new DataView(padded.buffer);
  view.setUint32(paddedLength - 8, Math.floor(source.byteLength / 0x20000000), false);
  view.setUint32(paddedLength - 4, (source.byteLength << 3) >>> 0, false);
...
export async function hashBytes(bytes, subtle = globalThis.crypto?.subtle) {
  if (!subtle?.digest) return sha256Fallback(bytes);
